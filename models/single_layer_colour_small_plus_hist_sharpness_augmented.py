...
from lib.transforms import (
    augmented,
    resize_small_colour_plus_hist_plus_sharp,
    default_augment,
)
...
class Model(TrashModel):
    ...
    MIN_DELTA = 1e-6
    transform = staticmethod(resize_small_colour_plus_hist_plus_sharp)
    ...
    def train(self) -> None:
        (X_training, Y_training), (X_validation, Y_validation), (X_test, Y_test) = (
            load_stratified_data(
                self.preprocess,
                train_preprocess=augmented(self.preprocess, default_augment),
            )
        )
        ...
        loss_history: list[tuple[int, float, float]] = []  # (epoch, train, val)
