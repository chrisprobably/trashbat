...
from lib.transforms import (
    MEDIUM_IMG_SIZE,
    resize_med_colour_plus_hist_plus_sharp,
    augmented,
    extended_med_augment,
)
...
class Model(TrashModel):
    ...
    PATIENCE = 500
    MIN_DELTA = 1e-6
    transform = staticmethod(resize_med_colour_plus_hist_plus_sharp)
    ...
    def train(self) -> None:
        (X_training, Y_training), (X_validation, Y_validation), (X_test, Y_test) = (
            load_stratified_data(
                self.preprocess,
                train_preprocess=augmented(self.preprocess, extended_med_augment),
            )
        )
        ...
        for epoch in range(self.MAX_ITERATIONS):
            ...
            mask = (
                torch.rand_like(hidden1) < self.DROPOUT_KEEP_PROB
            ).float() / self.DROPOUT_KEEP_PROB
            hidden1 = hidden1 * mask
            hidden2 = torch.relu(torch.mm(hidden1, weights2) + bias2)
            mask = (
                torch.rand_like(hidden2) < self.DROPOUT_KEEP_PROB
            ).float() / self.DROPOUT_KEEP_PROB
