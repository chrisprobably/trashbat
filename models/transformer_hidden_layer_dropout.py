...
class Model(TrashModel):
    ...
    INPUT_LAYER_SIZE = 3 * PATCH_SIZE * PATCH_SIZE
    ...
    def preprocess(self, img: Image.Image) -> torch.Tensor:
        ...
        patches = t.unfold(1, self.PATCH_SIZE, self.PATCH_SIZE).unfold(
            2, self.PATCH_SIZE, self.PATCH_SIZE
        )
        patches = patches.permute(1, 2, 0, 3, 4).contiguous()
        return patches.view(self.NUM_PATCHES, self.INPUT_LAYER_SIZE)
    ...
    def train(self) -> None:
        ...
        flat_size = self.NUM_PATCHES * self.ATTENTION_LAYER_SIZE

        # Attention projection matrices (patch_dim -> embed_dim)
        Wq = (
            torch.randn((self.INPUT_LAYER_SIZE, self.ATTENTION_LAYER_SIZE)) * 0.01
        ).requires_grad_(True)
        Wk = (
            torch.randn((self.INPUT_LAYER_SIZE, self.ATTENTION_LAYER_SIZE)) * 0.01
        ).requires_grad_(True)
        ...
        bq = torch.zeros(self.ATTENTION_LAYER_SIZE, requires_grad=True)
        bk = torch.zeros(self.ATTENTION_LAYER_SIZE, requires_grad=True)
        bv = torch.zeros(self.ATTENTION_LAYER_SIZE, requires_grad=True)

        # MLP layers on flattened attention output
        W1 = (torch.randn((flat_size, self.HIDDEN_LAYER_SIZE)) * 0.01).requires_grad_(
            True
        )
        b1 = torch.zeros(self.HIDDEN_LAYER_SIZE, requires_grad=True)
        W2 = (
            torch.randn((self.HIDDEN_LAYER_SIZE, len(CLASSES))) * 0.01
        ).requires_grad_(True)
        b2 = torch.zeros(len(CLASSES), requires_grad=True)

        params = [Wq, Wk, Wv, bq, bk, bv, W1, b1, W2, b2]

        def run(X: torch.Tensor, training: bool = False) -> torch.Tensor:
            attended = self._attend(X, Wq, Wk, Wv, bq, bk, bv)
            flat = attended.reshape(attended.shape[0], -1)
            hidden = torch.relu(torch.mm(flat, W1) + b1)
            if training:
                mask = (
                    torch.rand_like(hidden) < self.DROPOUT_KEEP_PROB
                ).float() / self.DROPOUT_KEEP_PROB
                hidden = hidden * mask
            return torch.mm(hidden, W2) + b2
        ...
        print(
            ...
            f"Patches: {self.NUM_PATCHES}, Patch size: {self.PATCH_SIZE}, Num patches/side: {self.NUM_PATCHES_PER_SIDE}, Attention Layer: {self.ATTENTION_LAYER_SIZE}, Hidden: {self.HIDDEN_LAYER_SIZE})..."
        )

        for epoch in range(self.MAX_ITERATIONS):
            logits = run(X_training, training=True)
